...
constexpr int IVF_MPI_OMP_NLIST = 256;
constexpr int IVF_MPI_OMP_TRAIN_ITER = 10;
constexpr int IVF_MPI_OMP_TRAIN_POINT = 8192;

inline float ivf_mpi_omp_ip_cal(float* b1,float *b2,size_t vecdim){
...
inline float ivf_mpi_omp_l2_cal(float* b1,float *b2,size_t vecdim){
    simd8float32 sum_simd;
    size_t i=0;
    for(;i+8<=vecdim;i+=8){
        simd8float32 v1(b1+i);
...
inline float ivf_mpi_omp_exact_cal(float* b1,float *b2,size_t vecdim){
    return 1.0f - ivf_mpi_omp_ip_cal(b1,b2,vecdim);
}

struct ivf_mpi_omp_t{
    float d; uint32_t id;
    bool operator<(const ivf_mpi_omp_t& o) const{return d<o.d;}
};

struct ivf_mpi_omp_ans_t{
    float d;
    uint32_t id;
};

struct IVFMPIOMPIndex{
    int nlist = IVF_MPI_OMP_NLIST;
    int dim = 0;
    int train_iter = IVF_MPI_OMP_TRAIN_ITER;
    bool ready = false;
...
#pragma omp parallel for schedule(static)
            for(int64_t i=0;i<(int64_t)point_num;i++){
                ...
                float best_dis = ivf_mpi_omp_l2_cal(cur, centroids.data(), dim);
                int best_id = 0;
                for(int c=1;c<nlist;c++){
                    float dis = ivf_mpi_omp_l2_cal(cur, centroids.data() + (size_t)c * dim, dim);
                    if(dis < best_dis){
                        best_dis = dis;
                        best_id = c;
                    }
...
#pragma omp parallel for schedule(static)
        for(int64_t i=0;i<(int64_t)local_num;i++){
            size_t gid = local_begin + (size_t)i;
    ...
    void train(float* base, size_t base_number, size_t vecdim, int nlist_val=IVF_MPI_OMP_NLIST, int iter=IVF_MPI_OMP_TRAIN_ITER, int train_point=IVF_MPI_OMP_TRAIN_POINT){
        init_mpi();
    ...
    std::priority_queue<std::pair<float, uint32_t> > query(float* query, size_t k, int nprobe){
        ...
        std::priority_queue<ivf_mpi_omp_t> coarse_q;
        for(int c=0;c<nlist;c++){
            float dis = ivf_mpi_omp_l2_cal(query, centroids.data() + (size_t)c * dim, dim);
        ...
        std::sort(probe_ids.begin(), probe_ids.end(), [&](uint32_t a, uint32_t b){
...
#pragma omp parallel for schedule(dynamic,1)
        for(int64_t pi=0;pi<(int64_t)probe_ids.size();pi++){
            int tid = omp_get_thread_num();
            uint32_t cid = probe_ids[(size_t)pi];
            for(size_t j=0;j<lists[cid].size();j++){
                uint32_t id = lists[cid][j];
                float sec = ivf_mpi_omp_exact_cal(base_data + (size_t)id * dim, query, dim);
        ...
        std::vector<ivf_mpi_omp_ans_t> send_buf(k);
        ...
        if(world_size==1){
            for(size_t i=0;i<send_buf.size();i++){
                if(send_buf[i].d>=1e29f) continue;
                q.push({send_buf[i].d, send_buf[i].id});
            }
            return q;
        }

        std::vector<ivf_mpi_omp_ans_t> recv_buf;
        if(rank==0) recv_buf.resize((size_t)world_size * k);

        MPI_Gather(send_buf.data(), (int)(k * sizeof(ivf_mpi_omp_ans_t)), MPI_BYTE,
                   rank==0 ? recv_buf.data() : nullptr, (int)(k * sizeof(ivf_mpi_omp_ans_t)), MPI_BYTE,
                   0, MPI_COMM_WORLD);
...
static IVFMPIOMPIndex g_ivf_mpi_omp_index;

inline void ivf_mpi_omp_train(float* base, size_t base_number, size_t vecdim, int nlist=IVF_MPI_OMP_NLIST, int iter=IVF_MPI_OMP_TRAIN_ITER, int train_point=IVF_MPI_OMP_TRAIN_POINT){
    g_ivf_mpi_omp_index.train(base, base_number, vecdim, nlist, iter, train_point);
}

inline std::priority_queue<std::pair<float, uint32_t> > ivf_mpi_omp_search(float* base, float* query, size_t base_number, size_t vecdim, size_t k, int nprobe){
    if(!g_ivf_mpi_omp_index.ready || g_ivf_mpi_omp_index.base_data!=base || g_ivf_mpi_omp_index.base_num!=base_number || g_ivf_mpi_omp_index.dim!=(int)vecdim){
        g_ivf_mpi_omp_index.train(base, base_number, vecdim);
    }
    return g_ivf_mpi_omp_index.query(query, k, nprobe);
}
