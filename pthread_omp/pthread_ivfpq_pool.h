...
inline void* ivfpq_pthread_pool_worker(void* ptr);
...
struct IVFPQPthreadIndex{
    ...
    std::vector<ivfpq_pthread_pool_arg> pool_args;

    float* pool_assign_base = nullptr;
    uint32_t* pool_assign_ids = nullptr;
    int* pool_assign_output = nullptr;
    bool pool_assign_use_ids = false;

    float* pool_sub_base = nullptr;
    std::vector<uint32_t>* pool_sub_train_id = nullptr;

    float* pool_encode_base = nullptr;

    uint32_t* pool_adc_cand = nullptr;
    size_t pool_adc_p = 0;
    std::vector<std::priority_queue<ivfpq_pthread_t> > pool_adc_local;

    float* pool_exact_query = nullptr;
    uint32_t* pool_exact_ids = nullptr;
    size_t pool_exact_k = 0;
    std::vector<std::priority_queue<std::pair<float, uint32_t> > > pool_exact_local;

    IVFPQPthreadIndex(){
        pthread_mutex_init(&pool_mutex, nullptr);
        pthread_cond_init(&pool_cond, nullptr);
        pthread_cond_init(&pool_done, nullptr);
    }
    ...
    ~IVFPQPthreadIndex(){
        ...
        pthread_cond_destroy(&pool_done);
    }
    ...
    void stop_pool(){
        ...
        for(int t=0;t<pool_thread_num;t++) pthread_join(pool_threads[t], nullptr);

        pool_threads.clear();
        pool_args.clear();
        pool_adc_local.clear();
        pool_exact_local.clear();
        pool_thread_num = 0;
        pool_task = 0;
        pool_working = 0;
        pool_epoch = 0;
    ...
    void start_pool(int tn){
        if(tn<=1) return;
        if(pool_ready && pool_thread_num==tn) return;

        stop_pool();
        pool_ready = true;
        pool_thread_num = tn;
        pool_threads.resize(tn);
        pool_args.resize(tn);
        pool_adc_local.resize(tn);
        pool_exact_local.resize(tn);

        for(int t=0;t<tn;t++){
            pool_args[t] = {this, t};
            pthread_create(&pool_threads[t], nullptr, ivfpq_pthread_pool_worker, &pool_args[t]);
        }
    }

    void finish_pool_task(){
        pthread_mutex_lock(&pool_mutex);
        pool_working--;
        if(pool_working==0){
            pool_task = 0;
    ...
    void run_pool_task(int task, size_t total){
        if(total==0 || pool_thread_num<=1) return;

        pthread_mutex_lock(&pool_mutex);
        pool_total = total;
        pool_task = task;
        pool_working = pool_thread_num;
        pool_epoch++;
        pthread_cond_broadcast(&pool_cond);
        while(pool_working!=0) pthread_cond_wait(&pool_done, &pool_mutex);
        pthread_mutex_unlock(&pool_mutex);
    }

    void pool_run(int tid, int task){
        size_t begin = pool_total * (size_t)tid / pool_thread_num;
        size_t end = pool_total * (size_t)(tid + 1) / pool_thread_num;

        if(task==1){
            assign_ivf_range(pool_assign_base, pool_assign_ids, begin, end, pool_assign_output, pool_assign_use_ids);
        }else if(task==2){
            train_pq_subspace_range(pool_sub_base, pool_sub_train_id, (int)begin, (int)end);
        }else if(task==3){
            encode_range(pool_encode_base, begin, end);
        }else if(task==4){
            adc_range(pool_adc_cand, begin, end, pool_adc_p, pool_adc_local[tid]);
        }else if(task==5){
            exact_range(pool_exact_query, pool_exact_ids, begin, end, pool_exact_k, pool_exact_local[tid]);
        }
    }
    ...
    void parallel_assign_ivf(float* base, uint32_t* ids, size_t total, int* output, bool use_ids){
        ...
        start_pool(tn);
        pool_assign_base = base;
        pool_assign_ids = ids;
        pool_assign_output = output;
        pool_assign_use_ids = use_ids;
        run_pool_task(1, total);
    }
    ...
    void encode_pq(float* base){
        ...
        start_pool(tn);
        pool_encode_base = base;
        run_pool_task(3, base_num);
    }
    ...
    std::priority_queue<std::pair<float, uint32_t> > query(float* query, size_t k, int nprobe, size_t p){
        ...
        if(cand.empty()) return q;
        if(tn<=1){
            adc_range(cand.data(), 0, cand.size(), p, q1);
        }else{
            start_pool(tn);
            pool_adc_cand = cand.data();
            pool_adc_p = p;
            pool_adc_local.clear();
            pool_adc_local.resize(tn);
            run_pool_task(4, cand.size());

            for(int t=0;t<tn;t++){
                while(!pool_adc_local[t].empty()){
                    ivfpq_pthread_t cur = pool_adc_local[t].top();
                    pool_adc_local[t].pop();
        ...
        start_pool(tn);
        pool_exact_query = query;
        pool_exact_ids = id_vec.data();
        pool_exact_k = k;
        pool_exact_local.clear();
        pool_exact_local.resize(tn);
        run_pool_task(5, id_vec.size());

        for(int t=0;t<tn;t++){
            while(!pool_exact_local[t].empty()){
                std::pair<float, uint32_t> cur = pool_exact_local[t].top();
                pool_exact_local[t].pop();
                if(q.size()<k){
                    q.push(cur);
...
inline void* ivfpq_pthread_pool_worker(void* ptr){
    ivfpq_pthread_pool_arg* arg = (ivfpq_pthread_pool_arg*)ptr;
    IVFPQPthreadIndex* index = arg->index;
    int tid = arg->tid;
    int last_epoch = 0;

    while(true){
        pthread_mutex_lock(&index->pool_mutex);
        while(index->pool_epoch==last_epoch && !index->pool_stop){
            pthread_cond_wait(&index->pool_cond, &index->pool_mutex);
        }
        if(index->pool_stop){
            pthread_mutex_unlock(&index->pool_mutex);
            break;
        }
        int task = index->pool_task;
        last_epoch = index->pool_epoch;
        pthread_mutex_unlock(&index->pool_mutex);

        index->pool_run(tid, task);
        index->finish_pool_task();
    }
