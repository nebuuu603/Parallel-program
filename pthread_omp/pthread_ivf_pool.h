...
constexpr int IVF_PTHREAD_THREAD = 8;
...
inline void* ivf_pthread_pool_worker(void* ptr);
...
struct IVFPthreadIndex{
    ...
    std::vector<ivf_pthread_pool_arg> pool_args;
    ...
    float* pool_query = nullptr;
    uint32_t* pool_query_cand = nullptr;
    size_t pool_query_k = 0;
    std::vector<std::priority_queue<std::pair<float, uint32_t> > > pool_query_local;
    ...
    void start_pool(int tn){
        ...
        pool_args.resize(tn);
        pool_query_local.resize(tn);

        for(int t=0;t<tn;t++){
            pool_args[t] = {this, t};
            pthread_create(&pool_threads[t], nullptr, ivf_pthread_pool_worker, &pool_args[t]);
        }
    }
    ...
    void pool_run(int tid, int task){
        ...
        if(task==1){
            assign_range(pool_assign_base, pool_assign_ids, begin, end, pool_assign_output, pool_assign_use_ids);
        }else if(task==2){
            query_range(pool_query, pool_query_cand, begin, end, pool_query_k, pool_query_local[tid]);
        }
    }
    ...
    std::priority_queue<std::pair<float, uint32_t> > query(float* query, size_t k, int nprobe){
        ...
        start_pool(tn);
        pool_query = query;
        pool_query_cand = cand.data();
        pool_query_k = k;
        pool_query_local.clear();
        pool_query_local.resize(tn);
        run_pool_task(2, cand.size());

        for(int t=0;t<tn;t++){
            while(!pool_query_local[t].empty()){
                std::pair<float, uint32_t> cur = pool_query_local[t].top();
                pool_query_local[t].pop();
                if(q.size()<k){
                    q.push(cur);
...
inline void* ivf_pthread_pool_worker(void* ptr){
    ivf_pthread_pool_arg* arg = (ivf_pthread_pool_arg*)ptr;
    IVFPthreadIndex* index = arg->index;
    int tid = arg->tid;
    int last_epoch = 0;
