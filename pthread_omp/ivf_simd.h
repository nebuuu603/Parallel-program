...
constexpr int IVF_SIMD_NLIST = 256;
constexpr int IVF_SIMD_TRAIN_ITER = 10;
constexpr int IVF_SIMD_TRAIN_POINT = 8192;

inline float ivf_simd_ip_cal(float* b1,float *b2,size_t vecdim){
...
inline float ivf_simd_l2_cal(float* b1,float *b2,size_t vecdim){
    simd8float32 sum_simd;
    size_t i=0;
    for(;i+8<=vecdim;i+=8){
        simd8float32 v1(b1+i);
...
inline float ivf_simd_exact_cal(float* b1,float *b2,size_t vecdim){
    return 1.0f - ivf_simd_ip_cal(b1,b2,vecdim);
}

struct ivf_simd_t{
    float d; uint32_t id;
    bool operator<(const ivf_simd_t& o) const{return d<o.d;}
};

struct IVFSIMDIndex{
    int nlist = IVF_SIMD_NLIST;
    int dim = 0;
    int train_iter = IVF_SIMD_TRAIN_ITER;
    bool ready = false;
    ...
    void train(float* base, size_t base_number, size_t vecdim, int nlist_val=IVF_SIMD_NLIST, int iter=IVF_SIMD_TRAIN_ITER, int train_point=IVF_SIMD_TRAIN_POINT){
        ...
        for(int it=0;it<train_iter;it++){
            for(size_t i=0;i<point_num;i++){
                ...
                float best_dis = ivf_simd_l2_cal(cur, centroids.data(), dim);
                int best_id = 0;
                for(int c=1;c<nlist;c++){
                    float dis = ivf_simd_l2_cal(cur, centroids.data() + (size_t)c * dim, dim);
                    if(dis < best_dis){
                        best_dis = dis;
                        best_id = c;
                    }
    ...
    std::priority_queue<std::pair<float, uint32_t> > query(float* query, size_t k, int nprobe){
        ...
        std::priority_queue<ivf_simd_t> coarse_q;
        for(int c=0;c<nlist;c++){
            float dis = ivf_simd_l2_cal(query, centroids.data() + (size_t)c * dim, dim);
        ...
        while(!coarse_q.empty()){
            ...
            for(size_t j=0;j<lists[cid].size();j++){
                ...
                float sec = ivf_simd_exact_cal(base_data + (size_t)id * dim, query, dim);
                if(q.size()<k){
                    q.push({sec,id});
...
static IVFSIMDIndex g_ivf_simd_index;

inline void ivf_simd_train(float* base, size_t base_number, size_t vecdim, int nlist=IVF_SIMD_NLIST, int iter=IVF_SIMD_TRAIN_ITER, int train_point=IVF_SIMD_TRAIN_POINT){
    g_ivf_simd_index.train(base, base_number, vecdim, nlist, iter, train_point);
}

inline std::priority_queue<std::pair<float, uint32_t> > ivf_simd_search(float* base, float* query, size_t base_number, size_t vecdim, size_t k, int nprobe){
    if(!g_ivf_simd_index.ready || g_ivf_simd_index.base_data!=base || g_ivf_simd_index.base_num!=base_number || g_ivf_simd_index.dim!=(int)vecdim){
        g_ivf_simd_index.train(base, base_number, vecdim);
    }
    return g_ivf_simd_index.query(query, k, nprobe);
}
