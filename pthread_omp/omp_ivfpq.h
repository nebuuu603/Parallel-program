...
constexpr int IVFPQ_OMP_NLIST = 256;
constexpr int IVFPQ_OMP_M = 4;
constexpr int IVFPQ_OMP_K = 256;
constexpr int IVFPQ_OMP_TRAIN_ITER = 10;
constexpr int IVFPQ_OMP_TRAIN_POINT = 8192;

inline float ivfpq_omp_ip_cal(float* b1,float *b2,size_t vecdim){
...
inline float ivfpq_omp_l2_cal(float* b1,float *b2,size_t vecdim){
    simd8float32 sum_simd;
    size_t i=0;
    for(;i+8<=vecdim;i+=8){
        simd8float32 v1(b1+i);
...
inline float ivfpq_omp_exact_cal(float* b1,float *b2,size_t vecdim){
    return 1.0f - ivfpq_omp_ip_cal(b1,b2,vecdim);
}

struct ivfpq_omp_t{
    float d; uint32_t id;
    bool operator<(const ivfpq_omp_t& o) const{return d<o.d;}
};

struct IVFPQOMPIndex{
    int nlist = IVFPQ_OMP_NLIST;
    int M = IVFPQ_OMP_M;
    int K = IVFPQ_OMP_K;
    int dim = 0;
    int sub_dim = 0;
    int train_iter = IVFPQ_OMP_TRAIN_ITER;
    bool ready = false;
...
#pragma omp parallel for
            for(int64_t i=0;i<(int64_t)point_num;i++){
                ...
                float best_dis = ivfpq_omp_l2_cal(cur, ivf_centroids.data(), dim);
                int best_id = 0;
                for(int c=1;c<nlist;c++){
                    float dis = ivfpq_omp_l2_cal(cur, ivf_centroids.data() + (size_t)c * dim, dim);
                    if(dis < best_dis){
                        best_dis = dis;
                        best_id = c;
                    }
...
#pragma omp parallel for
            for(int64_t i=0;i<(int64_t)train_id.size();i++){
                ...
                float best_dis = ivfpq_omp_l2_cal(cur, center[0].data(), sub_dim);
                int best_id = 0;
                for(int c=1;c<K;c++){
                    float dis = ivfpq_omp_l2_cal(cur, center[c].data(), sub_dim);
                    if(dis < best_dis){
                        best_dis = dis;
                        best_id = c;
                    }
...
#pragma omp parallel for
        for(int64_t i=0;i<(int64_t)base_num;i++){
            ...
            for(int m=0;m<M;m++){
                ...
                float best_dis = ivfpq_omp_l2_cal(cur_sub, pq_codebooks[m][0].data(), sub_dim);
                int best_id = 0;
                for(int c=1;c<K;c++){
                    float dis = ivfpq_omp_l2_cal(cur_sub, pq_codebooks[m][c].data(), sub_dim);
                    if(dis < best_dis){
                        best_dis = dis;
                        best_id = c;
                    }
                }
                pq_codes[(size_t)i * M + m] = (uint8_t)best_id;
            }
        }
    }
    ...
    void build_lut(float* query){
        for(int m=0;m<M;m++){
            for(int k=0;k<K;k++){
                ...
                pq_lut[m * K + k] = ivfpq_omp_ip_cal(q_sub, pq_codebooks[m][k].data(), sub_dim);
            }
        }
    }

    void train(float* base, size_t base_number, size_t vecdim, int nlist_val=IVFPQ_OMP_NLIST, int m=IVFPQ_OMP_M, int k=IVFPQ_OMP_K, int iter=IVFPQ_OMP_TRAIN_ITER, int train_point=IVFPQ_OMP_TRAIN_POINT){
    ...
    std::priority_queue<std::pair<float, uint32_t>> query(float* query_vec, size_t k, int nprobe, size_t p) {
        std::priority_queue<std::pair<float, uint32_t>> final_q;
        if (!ready || query_vec == nullptr) return final_q;
        std::priority_queue<ivfpq_omp_t> coarse_q;
        for (int c = 0; c < nlist; c++) {
            float dis = ivfpq_omp_l2_cal(query_vec, ivf_centroids.data() + (size_t)c * dim, dim);
        ...
        while (!coarse_q.empty()) {
            ...
            coarse_q.pop();
            cand.insert(cand.end(), lists[cid].begin(), lists[cid].end());
        }
        build_lut(query_vec);
        std::vector<std::priority_queue<ivfpq_omp_t>> local_queues(omp_get_max_threads());
    ...
    #pragma omp for
            for (int64_t i = 0; i < (int64_t)cand.size(); i++) {
                uint32_t id = cand[i];
                float sum = 0;
                const uint8_t* code = &pq_codes[(size_t)id * M];
                for (int m = 0; m < M; m++) {
                    sum += pq_lut[m * K + code[m]];
                }
                float ap = 1.0f - sum; 
                if (local_q.size() < p) {
                    local_q.push({ap, id});
                } else if (ap < local_q.top().d) {
                    local_q.pop();
                    local_q.push({ap, id});
                }
            }
        }
        std::priority_queue<ivfpq_omp_t> q1;
        for (auto& lq : local_queues) {
            while (!lq.empty()) {
                ivfpq_omp_t item = lq.top();
                lq.pop();
                if (q1.size() < p) q1.push(item);
                else if (item.d < q1.top().d) {
                    q1.pop();
                    q1.push(item);
                }
            }
        }
        std::vector<ivfpq_omp_t> top_p_cand;
        while(!q1.empty()){
            top_p_cand.push_back(q1.top());
            q1.pop();
        }
        std::vector<std::priority_queue<std::pair<float, uint32_t>>> final_local_queues(omp_get_max_threads());

    #pragma omp parallel
        {
            int tid = omp_get_thread_num();
            auto& local_f_q = final_local_queues[tid];

    #pragma omp for
            for (int64_t i = 0; i < (int64_t)top_p_cand.size(); i++) {
                uint32_t id = top_p_cand[i].id;
                float sec = ivfpq_omp_exact_cal(base_data + (size_t)id * dim, query_vec, dim);
                
                if (local_f_q.size() < k) {
                    local_f_q.push({sec, id});
                } else if (sec < local_f_q.top().first) {
                    local_f_q.pop();
                    local_f_q.push({sec, id});
                }
            }
        }
        for (auto& lq : final_local_queues) {
            while (!lq.empty()) {
                auto item = lq.top();
                lq.pop();
                if (final_q.size() < k) final_q.push(item);
                else if (item.first < final_q.top().first) {
                    final_q.pop();
                    final_q.push(item);
                }
            }
...
static IVFPQOMPIndex g_ivfpq_omp_index;

inline void ivfpq_omp_train(float* base, size_t base_number, size_t vecdim, int nlist=IVFPQ_OMP_NLIST, int m=IVFPQ_OMP_M, int ksub=IVFPQ_OMP_K, int iter=IVFPQ_OMP_TRAIN_ITER, int train_point=IVFPQ_OMP_TRAIN_POINT){
    g_ivfpq_omp_index.train(base, base_number, vecdim, nlist, m, ksub, iter, train_point);
}

inline std::priority_queue<std::pair<float, uint32_t> > ivfpq_omp_search(float* base, float* query, size_t base_number, size_t vecdim, size_t k, int nprobe, size_t p){
    if(!g_ivfpq_omp_index.ready || g_ivfpq_omp_index.base_data!=base || g_ivfpq_omp_index.base_num!=base_number || g_ivfpq_omp_index.dim!=(int)vecdim){
        g_ivfpq_omp_index.train(base, base_number, vecdim);
    }
    return g_ivfpq_omp_index.query(query, k, nprobe, p);
}
